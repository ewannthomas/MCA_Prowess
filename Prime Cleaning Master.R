...
Director_data_1<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2009.xlsx", sheet = "MasterProfile") %>% mutate(year=2009)
Director_data_2<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2010.xlsx", sheet = "MasterProfile")%>% mutate(year=2010)
Director_data_3<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2011.xlsx", sheet = "MasterProfile")%>% mutate(year=2011)
Director_data_4<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2012.xlsx", sheet = "MasterProfile")%>% mutate(year=2012)
Director_data_5<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2013.xlsx", sheet = "MasterProfile")%>% mutate(year=2013)
Director_data_6<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2014.xlsx", sheet = "MasterProfile")%>% mutate(year=2014)
Director_data_7<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2015.xlsx", sheet = "MasterProfile")%>% mutate(year=2015)
Director_data_8<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2016.xlsx", sheet = "MasterProfile")%>% mutate(year=2016)
Director_data_9<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2017.xlsx", sheet = "MasterProfile")%>% mutate(year=2017)
Director_data_10<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2018.xlsx", sheet = "MasterProfile")%>% mutate(year=2018)
Director_data_11<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2019.xlsx", sheet = "MasterProfile")%>% mutate(year=2019)
Director_data_12<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2020.xlsx", sheet = "MasterProfile")%>% mutate(year=2020)

Director_data<-bind_rows(Director_data_1,Director_data_2,Director_data_3,Director_data_4,Director_data_5,Director_data_6,Director_data_7,
                         Director_data_8,Director_data_9,Director_data_10,Director_data_11,Director_data_12) %>% 
  rename(Co_name = Company,
         ...
         Brief_profile = `Brief Profile`) %>% select(-Brief_profile) %>% 
  mutate(DoB = lubridate::dmy(DoB),
         ...
         Director_name=paste0(Director_first_name," ",Director_middle_name," ",Director_surname)) %>% select(-(4:7))

remove(Director_data_1,Director_data_2,Director_data_3,Director_data_4,Director_data_5,Director_data_6,Director_data_7,
       Director_data_8,Director_data_9,Director_data_10,Director_data_11,Director_data_12)


Dir_count<-Director_data %>% group_by(Co_name, year) %>% count(year) %>% rename(Director_count = n)
Director_data<-Director_data %>% left_join(., Dir_count)
remove(Dir_count)
...
Director_comm_5<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2013.xlsx", sheet = "Committee Details") %>% mutate(year=2013)
Director_comm_6<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2014.xlsx", sheet = "Committee Details") %>% mutate(year=2014)
Director_comm_7<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2015.xlsx", sheet = "Committee Details") %>% mutate(year=2015)
Director_comm_8<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2016.xlsx", sheet = "Committee Details") %>% mutate(year=2016)
Director_comm_9<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2017.xlsx", sheet = "Committee Details") %>% mutate(year=2017)
Director_comm_10<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2018.xlsx", sheet = "Committee Details") %>% mutate(year=2018)
Director_comm_11<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2019.xlsx", sheet = "Committee Details") %>% mutate(year=2019)
Director_comm_12<-readxl::read_excel("./Prime/Primedata/Indian Boards Data as on 31-Mar-2020.xlsx", sheet = "Committee Details") %>% mutate(year=2020)
...
# Prowess Board Data ------------------------------------------------------
load("./Prime/Cleaned Rdata/board_data.Rdata")
Prowess_Board<- board_data %>% mutate(year = lubridate::year(Slot_Date)) %>% 
  rename(Director_name = Director_Name)
remove(board_data)
...
AGM<-left_join(AGM_EGM_Resolutions, AGM_EGM, by=c("Co_name","NSE_symbol","Industry","cntrl_no","Type_of_meeting", "Date_of_event")) %>% 
  mutate(year = lubridate::year(Date_of_event))  

remove(AGM_EGM, AGM_EGM_Resolutions)
...
load("./Prime/Cleaned Rdata/Postal_Ballot_Data_Resolutions.Rdata")
Postal_Ballot_Data_Resolutions<-Postal_Ballot_Data_Resolutions %>% rename(Co_name = Company_name, 
                                                                          Cntrl_no = Central_num) %>% 
  select(-Record_date)

Postal_data<-right_join(Postal_ballots, Postal_Ballot_Data_Resolutions, by=c("Co_name", "NSE_symbol", "Industry", "Cntrl_no")) %>% mutate(TPP_votes_cast_shares_held_percent = as.numeric(case_when(
  TPP_votes_cast_shares_held_percent == "86.7209 |" ~ "86.7209",
  is.na(TPP_votes_cast_shares_held_percent) ~ NA_character_,
  TRUE ~ TPP_votes_cast_shares_held_percent
)))

remove(Postal_Ballot_Data_Resolutions, Postal_ballots)


# Prime Director Data Merging ------------------------------------------------------
...
Prime_Director<-left_join(Director_data, Director_data_remuneration, by = c("Co_name", "year","Person_Code")) %>% 
  select(-Director_name.y) %>% 
  rename(Director_name = Director_name.x) %>% 
  left_join(., Committee_details, by = c("Co_name", "year","Person_Code")) %>% 
  select(-Position.y, -Director_name.y) %>% 
  rename(Director_name = Director_name.x)

remove(Director_data, Director_data_remuneration, Committee_details)
